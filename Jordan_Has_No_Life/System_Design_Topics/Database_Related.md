...
2. [ACID Database Transactions](#acid-database-transactions)
   - [Atomicity](#atomicity)
   - [Consistency](#consistency)
   - [Isolation](#isolation---important)
   - [Durability](#durability)
   - [More on ACID](#more-on-acid)

3. [Race Conditions](#some-examples-of-race-conditions)
   - [Dirty Write](#dirty-write-writing-over-uncommitted-values)
   - [Dirty Reads](#dirty-reads-reading-uncommitted-data)
   - [Non Repeatable Read and Read Skew](#non-repeatable-read-and-read-skew)
   - [Write Skew and Phantom Writes](#write-skew-and-phantom-writes)
   - [Lost Update](#lost-update)

4. [Concurrency Control Methods](#achieving-acid-serial-execution)
   - [Serial Execution](#achieving-acid-serial-execution)
...
5. [Storage Formats](#column-oriented-storage)
   - [Column Oriented Storage](#column-oriented-storage)
   - [Advantages of Column Oriented Storage](#advantages-of-column-oriented-storage)
   - [Disadvantages of Column Oriented Storage](#disadvantages-of-column-oriented-storage)

4. [Data Serialization Frameworks](#data-serialization-frameworks)
...
`Row Oriented Storage` would typically divide the rows into a bunch of chuncks and store those chuncks into files.
- Every entry of each row is adjacent to each other in disk

Imagine the table schema is name, email, company
- In `Column Oriented Storage` we take each column data in the order and store that data into a file (Name file, Email file, Company file).
- The data of column is adjacent to eachother on disk

**Row vs Column Oriented Storage:**
- Row orinted storage is better when we want all the values for a Row (eg. Web App)
- Column orinted storage is better when we want all the values for a Column (eg. analytics)

### Advantages of Column Oriented Storage

Column oriented storage is also good when we want to perform something like `Column Compression`
- Column Compression?
    - Column compression is easier to perform when the data in that column is similar to each other. Example: Value moves between a certain number range which we know - (Numbers between 1 to 5, etc.)
        - Image example stored in ./images/column_compression.png
    - We can perfomr bitmap encoding and then Run length encoding
    - Benifits: 
        - Less data to send over the network to a server
        - Can keep more data stored in memory of CPU Cache

We can also do `Dictionary Compression` in column oriented storage
- We take arbitrary strings of a finite set and compression them to only a few bits

Another feature of COS is `Predicate Push Down`
- A feature of something called Parquet. Parquet is a open sorce Apache file format that allows us to perform COS.

### Disadvantages of Column Oriented Storage

Every Column must have the same sort order in each column file (to map with each other)
- If feasible, we can have multiple copies of one column file and sort different copies in required manner. Keep Original Copy.

Each writes need to go to many different places. Unless..
- Write rows to LSM tree -> Eventually export to column oriented files.

## Data Serialization Frameworks

Serialization Libraries -> Use a pre-defined data schema to reduce the size of the data.
- `Google's Protocol Buffers (Protobuf), Apaches Thrift`

So now the data is stored in the database in serialized way and every time we fetch it we have to deserialize it.
- This computation penalty is a trade off for the less storage achieved from serialization
- Another issue comes when we want to add a new field.

Protobuf and Thrift require knowledge of schema at complie time
- What if our data can change without us knowing in advance?

`Apache Avro` creates database schema on the fly based off of column names.
